...
def cmdLineParse():
    ...
    parser.add_argument('-idir', dest='idir', type=str, required=True,
            help = 'interferogram directory containing the burst interferograms')
...
if __name__ == '__main__':
    ...
    ibursts = sorted(glob.glob(os.path.join(inps.idir, '*.int')))

    nmb = len(mbursts) #number of master bursts
    nsb = len(sbursts) #number of slave bursts
    nib = len(ibursts) #number of burst interferograms
    ...
    for i in range(nb):
        ...
        int_looked = 'int_%02d_%drlks_%dalks.int' % (i+1,inps.rlks,inps.alks)
        cmd = "{}/look.py -i {} -o {} -r {} -a {}".format(SCR_DIR,
            ibursts[i], 
            int_looked,
            inps.rlks,
        ...
        int_looked_geo = 'int_%02d_%drlks_%dalks.int.geo' % (i+1,inps.rlks,inps.alks)
        cmd = "{}/geo_with_ll.py -input {} -output {} -lat {} -lon {} -bbox {} -ssize {} -rmethod {}".format(SCR_DIR,
            int_looked, 
            int_looked_geo,
            lat_looked,
        ...
        cor_looked_geo = 'cor_%02d_%drlks_%dalks.cor.geo' % (i+1,inps.rlks,inps.alks)
        cmd = "{}/geo_with_ll.py -input {} -output {} -lat {} -lon {} -bbox {} -ssize {} -rmethod {}".format(SCR_DIR,
            cor_looked, 
            cor_looked_geo,
            lat_looked,
